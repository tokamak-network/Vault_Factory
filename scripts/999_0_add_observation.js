...
async function main() {
  ...
  let contract = await ethers.getContractAt(UniswapV3PoolAbi.abi, uniswapInfo.wtonTosPool, ethers.provider);

  let slot = await contract.connect(deployer).slot0();
  console.log(slot);

  await contract.connect(deployer).increaseObservationCardinalityNext(2);
