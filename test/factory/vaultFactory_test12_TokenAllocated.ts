...
describe("VaultFactory", () => {
    ...
    let totalAmountOneWei = 1
    
    let totalAmountOneEther = 1
    const totalAmountEther = BigNumber.from(totalAmountOneEther).mul(BigNumber.from(BASE_TEN).pow(decimals))
    ...
    describe("TONVault test", async () => {
        ...
        it("check the input totalAllocated one wei", async () => {
            let curBlock = await ethers.provider.getBlock();
            claim1Time = curBlock.timestamp + (60*5);
            claim2Time = curBlock.timestamp + (60*8);
            claim3Time = curBlock.timestamp + (60*15);
            ...
            await expect(TONVaultLogic.connect(person2).initialize(
                ...
                [claim1Time,claim2Time,claim3Time,claim4Time,claim5Time,claim6Time],
                [claim1,claim2,claim3,claim4,claim5,claim6]
            )).to.be.revertedWith("need the totalAmount 1 token");
        })

        it("check the initialize after input token from Owner", async ()  => {
            let curBlock = await ethers.provider.getBlock();
            claim1Time = curBlock.timestamp + (60*5);
            claim2Time = curBlock.timestamp + (60*8);
            claim3Time = curBlock.timestamp + (60*15);
            ...
            console.log("claimTimes: ", claim1Time);

            expect(await TONVaultLogic.totalAllocatedAmount()).to.equal(totalAmountEther);
            expect(await TONVaultLogic.totalClaimCounts()).to.equal(1);
        })

        it("check the initialize after input token from proxyOwner", async ()  => {
            let curBlock = await ethers.provider.getBlock();
            claim1Time = curBlock.timestamp + (60*5);
            claim2Time = curBlock.timestamp + (60*8);
            claim3Time = curBlock.timestamp + (60*15);
            ...
            await TONVaultLogic.connect(proxyAdmin).ownerSetting(
                totalAmount,
                totalClaim,
                [claim1Time,claim2Time,claim3Time,claim4Time,claim5Time,claim6Time],
                [claim1,claim2,claim3,claim4,claim5,claim6]
            ...
            console.log("claimTimes: ", tx)
            console.log("nowclaimTimes: ", claim1Time)
