    ...
    describe("typeCVault test", () => {
        describe("setting", () => {
            ...
            it("initialize", async () => {
                ...
                let tx5 = await typeCVaultContract.claimAmounts(36);
                expect(tx5).to.equal(claim37)
            })
        ...
        describe("claim", () => {
            ...
            it("claim for round37", async () => {
                ...
                let claimAfter = await mockToken.balanceOf(account5.address)
                let claimAfterAmount2 = 7000000
                let claimAfterAmount3 = claimAfterAmount2 + claim37Amount
                
                let bigAfterAmount = BigNumber.from(claimAfterAmount3).mul(BigNumber.from(BASE_TEN).pow(decimals))
