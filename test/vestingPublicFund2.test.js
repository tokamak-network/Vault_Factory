...
let UniswapV3PoolABI = require('../abis/UniswapV3Pool.json');
let NonfungiblePositionManagerABI = require('../abis/NonfungiblePositionManager.json');
let TONToken = require('../abis/TOS.json');
const {getUniswapInfo} = require('./config_info.js');
...
describe("VestingPublicFund", function () {
    ...
    before(async function () {
        ...
        await hre.ethers.provider.send("hardhat_impersonateAccount",[config.tosAdminAddress]);
        TosAdmin = await ethers.getSigner(config.tosAdminAddress);

    });

    it("create tokenA : projectToken", async function () {
    ...
    it("set UniswapV3Factory  ", async function () {
        uniswapV3Factory = new ethers.Contract(config.uniswapV3FactoryAddress, UniswapV3FactoryABI.abi, admin1 );

    });

    it("set tonContract  ", async function () {
        tonContract = new ethers.Contract(config.tonAddress, ERC20AToken.abi, admin1 );
    ...
    describe("VestingPublicFundFactory   ", function () {
        ...
        it("0-3. setBaseInfo : when not admin, fail ", async function () {
            ...
            await expect(
                vestingPublicFundFactory.connect(user2).setBaseInfo(
                    [
                        config.tonAddress,
                        config.tosAddress,
                        info.daoAddress,
        ...
        it("0-3. setBaseInfo ", async function () {
            ...
            expect(await vestingPublicFundFactory.token()).to.be.eq(config.tonAddress);
            expect(await vestingPublicFundFactory.daoAddress()).to.be.eq(info.daoAddress);
            expect(await vestingPublicFundFactory.uniswapV3Factory()).to.be.eq(config.uniswapV3FactoryAddress);
        });
    ...
    describe("VestingPublicFund :  only ProxyOwner ", function () {
        ...
        it("3-1. changeAddr ", async function () {
            ...
            expect(await vestingPublicFund.token()).to.be.eq(config.tonAddress);
            expect(await vestingPublicFund.tosToken()).to.be.eq(info.tokenAddress);
            expect(await vestingPublicFund.projectToken()).to.be.eq(info.tokenAddress);
            ...
            expect(await vestingPublicFund.token()).to.be.eq(config.tonAddress);
            expect(await vestingPublicFund.tosToken()).to.be.eq(config.tosAddress);
            expect(await vestingPublicFund.projectToken()).to.be.eq(info.tokenAddress);
            expect(await vestingPublicFund.receivedAddress()).to.be.eq(receivedAddress.address);
    ...
    describe("vestingPublicFund : Only Public Sale Vault ", function () {
        ...
        it("5-1. funding : if uniswap v3 TOS-projectToken pool didn't existe, fail ", async function () {

            expect(await vestingPublicFund.publicSaleVaultAddress()).to.be.eq(publicSaleVault.address);

            let amount = ethers.utils.parseEther("1000");

            let poolAddress = await vestingPublicFund.getPoolAddress();
            await expect(poolAddress).to.not.eq("0x0000000000000000000000000000000000000000");

            let code = await ethers.provider.getCode(poolAddress);
            expect(code).to.eq("0x");

            await expect(
                vestingPublicFund.connect(user2).funding(amount)
             ).to.be.revertedWith("pool's current sqrtPriceX96 is zero.");
        });
        it(" send token ", async function () {

            const amount0 = ethers.utils.parseEther("1000"); //ethers.utils.parseUnits("1000000", 18)
            const amount1 = ethers.utils.parseEther("1000");

            // project Token
            const tx0 = await tokenA.connect(tokenInfo.admin).transfer(admin1.address, amount0);
            await tx0.wait();

            const tx1 = await tokenA
                .connect(admin1)
                .approve(config.NonfungiblePositionManager, amount1);
            await tx1.wait();

            expect(await tokenA.balanceOf(admin1.address)).to.be.gte(amount0);

            // tos Token
            tosToken = new ethers.Contract(config.tosAddress, TOSToken.abi, ethers.provider);
            await tosToken.connect(TosAdmin).transfer(admin1.address, amount1);

            const tx2 = await tosToken
                .connect(admin1)
                .approve(config.NonfungiblePositionManager, amount1 );

            await tx2.wait();
            expect(await tosToken.balanceOf(admin1.address)).to.be.gte(amount1);

        });

        it(" create TOS-projectToken pool ", async function () {
            ...
            let tx = await uniswapV3Factory.connect(admin1).createPool(config.tosAddress, info.tokenAddress, 3000);

            await tx.wait();

            let code1 = await ethers.provider.getCode(poolAddress);
            expect(code1).to.not.eq("0x");

            let pool = new ethers.Contract(poolAddress, UniswapV3PoolABI.abi, admin1);

            let sqrtPrice = encodePriceSqrt(ethers.constants.One, ethers.constants.One);

            let tx2 = await pool.connect(admin1).initialize(sqrtPrice);
            await tx2.wait();

            let slot = await pool.slot0();
            expect(slot.sqrtPriceX96).to.be.gt(0);
            // expect(slot.tick).to.be.eq(0);
        });

        it(" increase liquidity TOS-projectToken pool ", async function () {

            const amount0 = ethers.utils.parseEther("100");
            const amount1 = ethers.utils.parseEther("100");

            const mintParams = {
                token0: config.tosAddress,
                token1: info.tokenAddress,
              ...
              const npm = new ethers.Contract(config.NonfungiblePositionManager, NonfungiblePositionManagerABI.abi, admin1);

            const tx = await npm.connect(admin1).mint(mintParams);
        ...
        it("5-1. funding : when publicSaleVault's TON balance is insufficient, fail", async function () {

            expect(await vestingPublicFund.publicSaleVaultAddress()).to.be.eq(publicSaleVault.address);
            // await vestingPublicFund.connect(publicSaleVault).funding(ethers.utils.parseEther("1000"));
        ...
        it("5-1. funding : when caller didn't approve, fail", async function () {
            ...
            // 프로젝트 토큰 tokenA , TOS , TON
            // await tokenA.connect(tokenInfo.admin).transfer(publicSaleVault.address, amount);
            await tonContract.connect(TosAdmin).transfer(publicSaleVault.address, amount);

            expect(await tonContract.balanceOf(publicSaleVault.address)).to.be.eq(amount);
        ...
        // it("evm_increaseTime to first vesting time", async function () {
        //     let firstClaimTime = await vestingPublicFund.claimTimes(ethers.constants.Zero)
        ...
        //     if ( firstClaimTime > block.timestamp) pass = firstClaimTime-block.timestamp;

        //     if (pass > 0) {
        //         ethers.provider.send("evm_increaseTime", [pass+100])   // add 26 seconds
        ...
        it("5-1. funding :  ", async function () {
            ...
            let balanceReceivedAddress = await tonContract.balanceOf(receivedAddress.address);

            let amount  = ethers.utils.parseEther("1000");
            let balance = await tonContract.balanceOf(publicSaleVault.address);
            if (balance.lt(amount)) {
                await tonContract.connect(TosAdmin).transfer(publicSaleVault.address, amount);
            }
            expect(await tonContract.balanceOf(publicSaleVault.address)).to.be.gte(amount);

            await tonContract.connect(publicSaleVault).approve(vestingPublicFund.address, amount);
            expect(await tonContract.allowance(publicSaleVault.address, vestingPublicFund.address)).to.be.gte(amount);
            ...
            if (pass > 0) {
                ...
                let currentCount = await vestingPublicFund.currentRound() ;

                expect(await vestingPublicFund.currentRound()).to.be.eq(ethers.constants.Zero);
                expect(await tonContract.balanceOf(vestingPublicFund.address)).to.be.eq(amount);
                expect(await tonContract.balanceOf(receivedAddress.address)).to.be.eq(balanceReceivedAddress);

            } else {
                let currentCount = await vestingPublicFund.currentRound() ;

                expect(await vestingPublicFund.currentRound()).to.be.eq(ethers.constants.One);
                expect(await tonContract.balanceOf(vestingPublicFund.address)).to.be.lt(amount);
                expect(await tonContract.balanceOf(receivedAddress.address)).to.be.gt(balanceReceivedAddress);
    ...
    describe("VestingPublicFund : Anyone can run ", function () {
        ...
        it("6-1/2/3 . claim", async function () {
            ...
            let balanceOfPrev = await tonContract.balanceOf(receivedAddress.address);
            ...
            expect(await tonContract.balanceOf(receivedAddress.address)).to.be.eq(balanceOfPrev.add(amount));
        });
